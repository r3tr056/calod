...
impl Config {
    ...
    pub fn from_env_or_file() -> Result<Self, ConfigError> {
        if let Ok(capacity) = env::var("CACHE_CAPACITY") {
            let cache_capacity: usize = capacity
                .parse()
                .map_err(|_| ConfigError::InvalidEnvVar("CACHE_CAPACITY".to_string()))?;
            let ttl_seconds = env::var("TTL_SECONDS").ok().and_then(|v| v.parse().ok());
            let log_level = env::var("LOG_LEVEL").unwrap_or_else(|_| "INFO".to_string());
            let eviction_strategy =
                env::var("EVICTION_STRATEGY").unwrap_or_else(|_| "LRU".to_string());
            let default_ttl = env::var("DEFAULT_TTL").ok().and_then(|v| v.parse().ok());
            let persistence_enabled =
                env::var("PERSISTENCE_ENABLED").unwrap_or_else(|_| "false".to_string()) == "true";
            let max_cache_size_bytes = env::var("MAX_CACHE_SIZE_BYTES")
                .ok()
                .and_then(|v| v.parse().ok());
            let log_file_path = env::var("LOG_FILE_PATH").ok();
            let metrics_enabled =
                env::var("METRICS_ENABLED").unwrap_or_else(|_| "false".to_string()) == "true";
    ...
    pub fn from_json(path: &str) -> Result<Self, ConfigError> {
        ...
        let config: Config =
            serde_json::from_str(&config_data).map_err(|_| ConfigError::JsonParseError)?;
        Ok(config)
    }
}
